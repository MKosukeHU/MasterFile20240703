...
def setup_data_loaders(raw_data, batch_size=128, use_cuda=True):
    # 生データのtorch tensorへの変換
    x_numpy = raw_data.astype(np.float32)
    ...
    if use_cuda:
        ...
        valid_loader = DataLoader(dataset, batch_size=batch_size, shuffle=False, pin_memory=True)

    return train_loader, valid_loader

#%%

raw_data = np.loadtxt("TestData/data2.txt")
train_loder, valid_loader = setup_data_loaders(raw_data)
...
#%%
# 学習の実行
raw_data = np.loadtxt("TestData/data2.txt") # 生データの読み込み
dataloader_train, dataloader_valid = setup_data_loaders(raw_data, batch_size=1000) # 学習用（train）と検証用（valid）のデータローダーを作成
# dataloader_train, dataloader_valid = raw_data[:,0], raw_data[:,1]
model = VAE(z_dim = 1, x_dim = 3).to(device) # モデルをインスタンス化し、GPUにのせる, .to(device)でGPUを動作
...
# print(f"use data = {dataloader_train}")
# print(f"data type = {type(dataloader_train)}")

#%%
for i in range(num_epochs):
    losses = []
    for x_batch in dataloader_train: # データローダーからデータを取り出す。
        x = x_batch[0].to(device)  # データをGPUにのせる
        print(f"batch data x ={x}")
...
model.eval()
with torch.no_grad():
    for x_batch in dataloader_valid:
        x = x_batch[0].to(device)  # データをGPUに転送
        _, _, y = model(x, device)
        y = y.view(x.size())  # yの形状をxと同じに調整
        # print(f"元データ : {x} \n 再構成データ : {y}")
        print(f"データの差（元データ - 再構成データ） : {x - y}")
